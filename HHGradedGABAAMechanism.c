...
static MYRIAD_FXN_METHOD_HEADER_GEN(MECH_FXN_RET, MECH_FXN_ARGS, HHGRADEDGABAAMECHANISM_OBJECT, INDIVIDUAL_MECH_FXN_NAME)
{
...
MYRIAD_FXN_METHOD_HEADER_GEN_NO_SUFFIX(DYNAMIC_INIT_FXN_RET, DYNAMIC_INIT_FXN_ARGS, HHGRADEDGABAAMECHANISM_INIT_FXN_NAME)
//void initHHGradedGABAAMechanism(int init_cuda)
{
	if (!HHGRADEDGABAAMECHANISM_CLASS)
	{
		...
		if (init_cuda)
		{
			...
			CUDA_CHECK_RETURN(
				cudaMemcpyToSymbol(
					(const void*) &MYRIAD_CAT(HHGRADEDGABAAMECHANISM_CLASS, MYRIAD_CAT(_, DEV_T)),
					&tmp_mech_c_t,
					sizeof(struct HHGRADEDGABAAMECHANISM_CLASS*),
					0,
					cudaMemcpyHostToDevice
	...
	if (!HHGRADEDGABAAMECHANISM_OBJECT)
	{
		HHGRADEDGABAAMECHANISM_OBJECT =
			myriad_new(
				...
				MECH_FXN_NAME_D, MYRIAD_CAT(HHGRADEDGABAAMECHANISM_OBJECT, MYRIAD_CAT(_, INDIVIDUAL_MECH_FXN_NAME)),
				0
			);
