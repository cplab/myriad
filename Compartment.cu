...
static void* CompartmentClass_cudafy(void* _self, int clobber)
{
	...
	CUDA_CHECK_RETURN(
		cudaMemcpyFromSymbol(
			(void**) &my_comp_fun,
			Compartment_cuda_compartment_fxn_t,
			sizeof(void*)
			)
		);
...
__device__ __constant__ struct Compartment* Compartment_dev_t = NULL;
__device__ __constant__ struct CompartmentClass* CompartmentClass_dev_t = NULL;
	...
	if (!CompartmentClass)
	{
		CompartmentClass = 
			myriad_new(
				   MyriadClass,
				   MyriadClass,
				   size_t(sizeof(struct CompartmentClass)),
